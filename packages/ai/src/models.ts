...
export function calculateCost<TApi extends Api>(model: Model<TApi>, usage: Usage): Usage["cost"] {
	...
	usage.cost.estimate = { input, output, cacheRead, cacheWrite, total };

	// Fill in zero fields with estimate (provider may have set actual values)
	if (usage.cost.input === 0) usage.cost.input = input;
	if (usage.cost.output === 0) usage.cost.output = output;
	if (usage.cost.cacheRead === 0) usage.cost.cacheRead = cacheRead;
	if (usage.cost.cacheWrite === 0) usage.cost.cacheWrite = cacheWrite;
	if (usage.cost.total === 0) usage.cost.total = total;
	return usage.cost;
}
/**
