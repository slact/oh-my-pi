...
export async function processResponsesStream<TApi extends Api>(
	...
	for await (const event of openaiStream) {
		if (event.type === "response.created") {
			...
			populateResponsesUsageFromResponse(output, response?.usage, response?.usage);
...
export function populateResponsesUsageFromResponse(
	...
	const hasActual = typeof rawUsage?.cost === "number" || typeof rawUsage?.is_byok === "boolean";
	output.usage = {
		...
		cost: {
			input: rawUsage?.upstream_inference_input_cost ?? 0,
			output: rawUsage?.upstream_inference_output_cost ?? 0,
			cacheRead: 0,
			cacheWrite: 0,
			total: rawUsage?.cost ?? 0,
			isByok: typeof rawUsage?.is_byok === "boolean" ? rawUsage.is_byok : undefined,
			hadActualCost: hasActual || undefined,
		},
	};
}
