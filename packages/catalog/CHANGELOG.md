...
- Fixed `calculateCost()` zeroing the aggregate `usage.cost.total` when callers already populated real cost shards (`hadActualCost`). The helper now keeps reported dollar amounts intact, backfills a missing total from the reported shards, and still stores the model-priced estimate alongside them.
